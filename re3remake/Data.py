...
class Data:
    ...
    def load_data(character=None, scenario=None):
        ...
        # Nightmare and Inferno share enemies_extended.json, so one constant
        # marker covers both. Kept in sync with Lookups.lua on the client.
        enemy_suffix_extended = " (N/I)"

        location_start = item_start = 3300000000 + character_offsets[character] + scenario_offsets[scenario]
        # Keep enemy IDs inside uint32 range. location_start+1_000_000_000
        # (4300000000) overflows uint32 and AP silently drops those LocationChecks.
        # Files use location_start+1000+; leave a gap after that for enemies.
        enemy_start = location_start + 2000
        ...
        ###
        # Add optional collectible-file locations
        ###

        file_location_table = load_data_file(character, scenario, "files.json")
        Data.file_table.extend([
            {
                **loc,
                "id": loc["id"] if loc.get("id") else location_start + 1000 + key,
                "region": loc["region"] + scenario_suffix,
                "character": character,
                "scenario": scenario,
                "difficulty": None
            }
            for key, loc in enumerate(file_location_table)
        ])
        ...
        # enemies_extended.json is the Nightmare/Inferno enemy list (not a
        # supplement). Generation uses it only for those difficulties via
        # _get_active_enemy_table(), so its rows own their ids and names
        # outright — nothing is inherited from enemies.json.
        #
        # Both difficulties read the same list, so these checks can't take a
        # per-difficulty region suffix the way the location tables do. The
        # marker goes on the name instead, which also keeps the 71 rows that
        # share a region + name with a standard enemy distinct in the
        # datapackage. Region must stay unsuffixed or create_regions drops the
        # check (no matching Region object).
        Data.enemy_table_extended.extend([
            {
                **enemy,
                "id": enemy["id"] if enemy.get("id") else enemy_remaining + key,
                "name": enemy["name"] + enemy_suffix_extended,
                "region": enemy["region"],
                "original_item": "__Enemy Kill Drop Placeholder__"
            }
