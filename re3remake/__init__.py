...
from .Options import RE3ROptions, re3remake_option_groups
...
class UmbrellaNet(WebWorld):
    ...
    tutorials = [Tutorial(
        ...
        "A guide for setting up RE3Remake to be played in Archipelago.",
        "English",
        "re3r_en.md",
        "setup/en",
        ["TheRealSolidusSnake"]
    )]
    option_groups = re3remake_option_groups
...
class ResidentEvil3Remake(World):
    ...
    # Enemy datapackage rules:
    # - Assisted / Standard / Hardcore use Data.enemy_table only at generation.
    # - Nightmare / Inferno use Data.enemy_table_extended only at generation.
    # The two lists are fully independent: extended rows carry their own ids
    # and their own " (N/I)" names, so neither can resolve onto the other.
    # The maps still list every possible check id, since the datapackage is
    # static and shared by every slot regardless of difficulty.
    location_id_to_name = {}
    location_name_to_id = {}
    location_name_to_location = {}

    for loc in (Data.location_table + Data.enemy_table
                + Data.file_table + Data.enemy_table_extended):
        stacked = RE3RLocation.stack_names(loc['region'], loc['name'])
        loc_id = loc['id']
        if loc_id not in location_id_to_name:
            location_id_to_name[loc_id] = stacked
        if stacked not in location_name_to_id:
            location_name_to_id[stacked] = loc_id
        if stacked not in location_name_to_location:
            location_name_to_location[stacked] = loc
    ...
    def generate_early(self):
        ...
        for key, val in getattr(self.multiworld, 're_gen_passthrough', {}).get(self.game, {}).items():
            ...
            # Slot data / Universal Tracker often pass pretty names ("All"),
            # while Choice.options keys are normalized ("all").
            if isinstance(val, int):
                option.value = val
                continue

            options_map = getattr(option, "options", {})
            if val in options_map:
                option.value = options_map[val]
                continue

            normalized = str(val).strip().lower().replace(" ", "_")
            if normalized in options_map:
                option.value = options_map[normalized]
                continue

            if hasattr(option, "from_text"):
                option.value = option.from_text(str(val)).value
                continue

            raise KeyError(f"Unknown value {val!r} for option {key}")
    ...
    def _get_active_enemy_table(self):
        """Enemy location set for this seed's difficulty.

        Assisted / Standard / Hardcore -> enemies.json
        Nightmare / Inferno            -> enemies_extended.json

        Extended is never mixed into Standard/Hardcore/Assisted pools.
        """
    ...
    def create_items(self, to_item_names=None):
        ...
        if grenades_enabled and handguns_enabled:
            raise RE3ROptionError(
                f"{self.player_name}'s RE3Remake cannot have both Oops All options enabled at the same time."
            )
        ...
        # traps (Parasite / Puke are Jill-only at runtime; this apworld is Jill-fixed)
        traps = []
    ...
    def create_item(self, item_name: str) -> Item:
        ...
        elif item.get('type', None) in ['Filler', 'File']:
            # Files unlock nothing in the multiworld; keep them filler.
    ...
    def get_filler_item_name(self) -> str:
        # Documents are always valid filler, even when Add Files as Locations
        # is off (that option only adds the pickup checks, not the item type).
        if not hasattr(self, "_filler_item_names"):
            self._filler_item_names = [
                item["name"]
                for item in Data.item_table
                if item.get("type") == "File"
            ] or ["Green Herb"]
        return self.random.choice(self._filler_item_names)
    ...
    def fill_slot_data(self) -> Dict[str, Any]:
        slot_data = {
            ...
            "speedy_enemy_mode": self._format_option_text(self.options.speedy_enemy_mode),
            "speedy_enemy_multiplier": self._format_option_text(self.options.speedy_enemy_multiplier),
            "invisible_enemy_mode": self._format_option_text(self.options.invisible_enemy_mode),
            "files_as_locations": self._format_option_text(self.options.add_files_as_locations)
        }
    ...
    def interpret_slot_data(self, slot_data: dict[str, Any]):
        ...
        if slot_data.get('files_as_locations') is not None:
            regen_values['add_files_as_locations'] = slot_data['files_as_locations']
        # Without this, UT regenerates with enemy kills off and never lists those locations.
        if slot_data.get('enemy_kills') is not None:
            regen_values['add_enemy_kills_as_locations'] = slot_data['enemy_kills']
        return regen_values
    ...
    def _get_locations_for_scenario(self, character, scenario) -> dict:
        ...
        if self._files_rando():
            locations_pool.update({
                file_location['id']: file_location
                for file_location in Data.file_table
                if not file_location.get('excluded', 0)
            })
    ...
    def _files_rando(self) -> bool:
        return self._format_option_text(self.options.add_files_as_locations) != "None"
